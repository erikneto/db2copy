...
module.exports = {
    ...
    init: (user,password,workspace,version_date) => {
        let newService = {
                    username: user,
                    password: password,
                    ...
                    Promise.all(promises).then(intents => {
                        ...
                        Promise.all(prmsTest).then(result => {
                                ...
                                e.forEach((i)=>{
                                    ...
                                    if(!intent){
                                        intent = {
                                            ...
                                            total: 0,
                                            correct: 0,   
                                            nts65: 0 //no threshold 65%
                                        }
                                        summary.push(intent);
                                    }
                                    intent.total++;
                                    intent.correct += (i.correctIntent === i.returnedIntent && i.confidence>=0.65)?1:0;
                                    intent.nts65 += i.correctIntent === i.returnedIntent?1:0;
                            ...
                            summary.forEach((e) => {
                                ...
                                correct += e.correct;
                                nts65 += e.nts65;
                                console.log("Intenção %s. Perguntas %d. Acertos %d. Acurácia %s%. Sem Threshold %s%",e.intent,e.total,e.correct,(e.correct/e.total*100).toFixed(2),(e.nts65/e.total*100).toFixed(2));
                            });
                            console.log("Total. Perguntas %d. Acertos %d. Acurácia %s%. Sem Threshold %s%",total,correct,(correct/total*100).toFixed(2),(nts65/total*100).toFixed(2));
