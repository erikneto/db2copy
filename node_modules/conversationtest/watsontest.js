                    ...
                    Promise.all(promises).then(intents => {
                        ...
                        Promise.all(prmsTest).then(result => {
                            ...
                            summary.forEach((e) => {
                                ...
                                e.confusionMatrix.forEach((cm,idx) => {
                                    console.log(summary[idx].intent,cm);
                                });
                            });
                    ...
                    if (!runningWSCreation) {
                        ...
                        conversationService.getWorkspace({ 'workspace_id': wsnew.workspace_id }, (err, result) => {
                            ...
                            if(wsnew.tries === 20){
                                ...
                                clearInterval(intervalId);
                                return reject({msg: "General error when training workspace"});
                            }
...
function createConfusionMatrix(intents,samples) {
    ...
    samples.forEach((s) => {
        confusionMatrix[_.findIndex(intents,(o) => {return o.intent === s.returnedIntent})]++;
    });
    return confusionMatrix;
}

function deleteWorkspace(workspace_id){
    conversationService.deleteWorkspace({ 'workspace_id': workspace_id }, (err, result) => {
        if (err) {
